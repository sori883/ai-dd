...
This fixture's Git operations need host support under the normal sandbox. The test-only tool ` + helper + ` request FILE submits a JSON host job and waits; use async polling for long jobs. It does not modify workflow state. Job shapes: {"Kind":"prepare","Units":["a","b"]} returns actual worker sessions/root; {"Kind":"workers","Units":["a","b"],"Intent":"ID"} starts those assigned workers concurrently and returns actual tests/commits; {"Kind":"integrate","Commit":"HASH"} integrates a worker commit; {"Kind":"prepare-review"} returns an independent read-only root/session; {"Kind":"review","Intent":"ID","Target":"HASH","Session":"SESSION","Root":"ROOT"} runs that assigned reviewer; {"Kind":"commit"} commits fixture artifacts and returns current HEAD. The commit host job commits all nonignored fixture files. Before preparing a reviewer checkout, commit fixture evidence only when the host helper needs it. Set config.verification_paths to source, tests and go.mod; use intent hash before and after actual tests and write the current SHA in the result JSON under aidlc/evidence/. Use one literal CLI command per tool call. Keep all JSON request drafts under aidlc/.runtime/ or .flow-queue/ so they do not change code targets. You must perform all intent/unit CLI operations yourself with actual revisions. Unit scopes are a.go+a_test.go, b.go+b_test.go, c.go+c_test.go; Use the assigned ordinary worker root. Host commits are fixture transport only. Submit unit result using verification_sha256, unit_id/run_id and unit-scope test results; integrate only after the manager files match. Register unit and final intent-scope results with current step_id, stage, verification_sha256 and per-run unit_id/command/exit_code/output_path. For c, prepare its root only after both dependencies integrate. Test proof files are in each worker .flow-proof; preserve relevant final evidence as an artifact. Do not claim AI performed host Git operations. Respond only after completed state. Do not bypass hooks or change Rules.`
	go func() {
