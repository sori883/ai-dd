...
// TestHookProbeHelper is the hook executable, using the current Go test
// binary so the probe does not install a runtime or copy credentials.
func TestHookProbeHelper(t *testing.T) {
	split := -1
	...
	observed := hookProbeEvent{Input: input, Files: hookProbeReadFiles(root)}
	...
	output := `{}`
	if input.Event == "PreToolUse" && input.Input.Command == hookProbeCommands[0] {
...
func TestHookProbeHelperProtocol(t *testing.T) {
	...
	for _, tc := range []struct{ name, event, command, want string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			input := hookProbeInput{Event: tc.event, Session: "test-session"}
			...
			defer cancel()
			cmd := exec.CommandContext(ctx, binary, "-test.run=^TestHookProbeHelper$", "--", "hook", root, evidence)
	...
	events, err := hookProbeLoadEvents(evidence)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 4 {
...
func TestHookProbeLive(t *testing.T) {
	...
	evidence, err := os.MkdirTemp("", "aidlc-hook-probe-")
	if err != nil {
		t.Fatal(err)
	}
	...
	root, err = hookProbeCanonicalRoot(root)
	if err != nil {
		t.Fatal(err)
	}
	...
	hookCommand := hookProbeQuote(binary) + " -test.run='^TestHookProbeHelper$' -- hook " + hookProbeQuote(root) + " " + hookProbeQuote(evidence)
...
1. exec_command cmd: ` + hookProbeCommands[0] + `
2. exec_command cmd: ` + hookProbeCommands[1] + `
3. exec_command cmd: ` + hookProbeCommands[2] + `
4. exec_command cmd: ` + hookProbeCommands[3] + ` (yield_time_ms=1; poll until terminal)
5. exec_command cmd: ` + hookProbeCommands[4] + ` (yield_time_ms=1; poll until terminal)
...
Then give a short final response. The Stop hook will request one acknowledgement. Do not modify probe-forbidden by another method. Do not create any extra files.`
	...
	args := []string{"exec", "--ignore-user-config", "--dangerously-bypass-hook-trust", "-s", "workspace-write", "-c", `approval_policy="never"`, "-m", "gpt-6-astra", "-c", `model_reasoning_effort="medium"`, "-c", hookProbeTrustConfig(root), "-C", root, "--json", prompt}
	...
	if err := hookProbeVerify(events, calls, hookProbeReadFiles(root)); err != nil {
		t.Fatalf("hook compatibility unproven: %v; raw evidence: %s", err, evidence)
	}
}

func hookProbeQuote(value string) string {
	return "'" + strings.ReplaceAll(value, "'", "'\"'\"'") + "'"
}
func strconvQuote(value string) string { encoded, _ := json.Marshal(value); return string(encoded) }
func hookProbeReadFiles(root string) map[string]string {
	files := map[string]string{}
	for _, name := range hookProbeFiles {
...
func hookProbeLoadEvents(dir string) ([]hookProbeEvent, error) {
	paths, err := filepath.Glob(filepath.Join(dir, "event-*.json"))
	if err != nil {
		return nil, err
	}
	sort.Strings(paths)
	var events []hookProbeEvent
	for _, path := range paths {
		raw, err := os.ReadFile(path)
		if err != nil {
			return nil, err
		}
		var event hookProbeEvent
...
func hookProbeCalls(raw []byte) ([]hookProbeCall, error) {
	...
	for scanner.Scan() {
		...
		case "custom_tool_call":
			...
			call, err := hookProbeLiteralCall(p.Input)
			if err != nil {
				return nil, err
			}
			call.ID = p.ID
		...
		case "custom_tool_call_output":
			...
			blocked := calls[at].Name == "exec_command" && hookProbeCommand(calls[at]) == hookProbeCommands[0]
			if blocked {
...
func TestHookProbeObservedTransport(t *testing.T) {
...
{"type":"response_item","payload":{"type":"custom_tool_call_output","call_id":"outer-poll","output":[{"type":"input_text","text":"Script completed\nWall time 0.0 seconds\nOutput:\n"},{"type":"input_text","text":"{\"exit_code\":0,\"output\":\"\"}"}]}}`
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if calls[0].ID != "outer-start" || calls[0].Name != "exec_command" || hookProbeCommand(calls[0]) != hookProbeCommands[3] || hookProbeRunning(calls[0].Output) != "42" {
				t.Fatalf("start = %+v", calls[0])
			}
			if calls[1].Name != "write_stdin" || calls[1].Arguments != `{"session_id":42}` || !hookProbeExited(calls[1].Output, 0) {
...
func TestHookProbeReplay(t *testing.T) {
	evidence := os.Getenv("AIDLC_HOOK_EVIDENCE")
	if evidence == "" {
		t.Skip("set AIDLC_HOOK_EVIDENCE to replay a preserved live recording")
	}
	events, err := hookProbeLoadEvents(evidence)
	if err != nil {
		t.Fatal(err)
	...
	if err := hookProbeVerify(events, calls, hookProbeReadFiles(filepath.Join(evidence, "repo"))); err != nil {
		t.Fatal(err)
	}
