...
func TestCodexStageDispatchHoldsWorkspaceLockAcrossSelectionAndAction(t *testing.T) {
	...
	codexStageAfterSelectionValidation = func() {
		...
		defer cancel()
		probeDone <- workspace.WithLock(probeContext, fixture.identity.ProjectPath(), func() error { return nil })
		<-continueDispatch
	}
	...
	case err := <-probeDone:
		if err == nil || !errors.Is(err, context.DeadlineExceeded) {
			t.Fatalf("same-project workspace probe = %v, want deadline while hidden action is held", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("same-project workspace probe did not finish")
	}
