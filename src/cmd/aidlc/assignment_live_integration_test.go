...
func TestAssignmentLive(t *testing.T) {
	...
	for _, mode := range []string{"allow-parallel", "deny", "missing-post", "save-failure"} {
		t.Run(mode, func(t *testing.T) {
			...
			hookCommand := hookProbeQuote(os.Args[0]) + " -test.run='^TestAssignmentLiveHook$' -- assignment-live-hook " + hookProbeQuote(root) + " " + hookProbeQuote(binary) + " " + hookProbeQuote(events) + " " + hookProbeQuote(mode)
			...
			processCommand := hookProbeQuote(os.Args[0]) + " -test.run='^TestAgentHookProbeProcess$' -- agent-process " + hookProbeQuote(processes) + " NONCE 15000"
			if mode == "allow-parallel" {
				processCommand = hookProbeQuote(os.Args[0]) + " -test.run='^TestAssignmentProcess$' -- assignment-process " + hookProbeQuote(processes) + " NONCE PEER 90000 15000"
			}
