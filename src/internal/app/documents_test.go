...
func TestIntentDocumentsCommands(t *testing.T) {
	...
	raw, err := s.Execute(cli.CommandRequest{Command: "intent", Action: "documents", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), File: draft})
	if err != nil {
		t.Fatal(err)
	}
	...
	raw, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "documents", Target: st.ID, Space: "default"})
	...
	raw, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "configure", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), File: draft})
	if err != nil {
		t.Fatal(err)
	}
	if err = json.Unmarshal(raw, &st); err != nil || len(st.Config.DocumentOutputs) != 1 {
	...
	if _, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "configure", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), File: draft}); err == nil {
...
func TestIntentDocumentsRoundtrip(t *testing.T) {
	...
	if _, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "documents", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), File: draft}); err != nil {
