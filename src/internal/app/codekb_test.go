...
func TestCodeKBMemory(t *testing.T) {
	...
	raw, err := s.Execute(cli.CommandRequest{Command: "memory", Action: "show", Target: "codekb/note", Space: "default"})
	if err != nil {
		t.Fatal(err)
	}
	...
	raw, err = s.Execute(cli.CommandRequest{Command: "memory", Action: "search", Target: "Memory", Space: "default"})
	if err != nil {
		t.Fatal(err)
	}
