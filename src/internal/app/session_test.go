...
func bind(t *testing.T, s Service, id string) {
	t.Helper()
	if _, err := s.Execute(cli.CommandRequest{Command: "session", Action: "bind", Target: id, Space: "default", Session: "session"}); err != nil {
		t.Fatal(err)
	}
}
func TestRulesFullTextAndLimit(t *testing.T) {
	s, _ := setup(t)
	raw, err := s.Execute(cli.CommandRequest{Command: "memory", Action: "rules", Space: "default"})
	...
	if _, err := s.Execute(cli.CommandRequest{Command: "memory", Action: "rules", Space: "default"}); err == nil {
...
func TestSessionMemoryBoundaries(t *testing.T) {
	...
	out, err := s.Execute(cli.CommandRequest{Command: "memory", Action: "search", Space: "other", Target: "Memory"})
...
func TestSessionMemoryShowOriginalHash(t *testing.T) {
	...
	out, err := s.Execute(cli.CommandRequest{Command: "memory", Action: "show", Target: "codekb/exact", Space: "default"})
	if err != nil {
		t.Fatal(err)
	}
