...
func TestFlowIntentCommand(t *testing.T) {
	...
	s := Service{Root: root}
	raw, err := s.Execute(cli.CommandRequest{Command: "intent", Action: "create", Target: "Work", Space: "default"})
	if err != nil {
		t.Fatal(err)
	}
	var st flow.State
	...
	raw, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "show", Target: st.ID, Space: "default"})
	if err != nil {
		t.Fatal(err)
	}
	if err := json.Unmarshal(raw, &st); err != nil {
...
func TestFlowHookSelectionRulesAndRecovery(t *testing.T) {
	...
	if _, err := s.Execute(cli.CommandRequest{Command: "session", Action: "bind", Target: st.ID, Space: "other", Session: "session", Recover: true}); err == nil {
		t.Fatal("foreign recover accepted")
	}
	if _, err := s.Execute(cli.CommandRequest{Command: "session", Action: "bind", Target: st.ID, Space: "default", Session: "session", Recover: true}); err != nil {
		t.Fatal(err)
	}
...
func TestFlowConfigurePreservesActiveAssignment(t *testing.T) {
	...
	_, err = (Service{Root: root}).Execute(cli.CommandRequest{Command: "intent", Action: "configure", Space: "default", Target: st.ID, Expect: strconv.FormatUint(st.Revision, 10), File: file})
	if err == nil {
...
func TestFlowInactiveWorkflowReadAndResume(t *testing.T) {
	for _, status := range []string{"completed", "waiting", "paused"} {
		t.Run(status, func(t *testing.T) {
			...
			_, err = s.Execute(cli.CommandRequest{Command: "intent", Action: action, Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), Reason: "retry", Step: "s02"})
			if err != nil {
				t.Fatal(err)
			}
