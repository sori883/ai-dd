...
func TestExecutionPlanCLIPendingHook(t *testing.T) {
	...
	for _, tc := range []struct {
		command string
		deny    bool
	}{{"/opt/aidlc intent hash " + st.ID + " --space default", false}, {"cat .agents/skills/okf-agent-memory/SKILL.md", false}, {"touch code.go", true}, {"cat .agents/skills/aidlc/SKILL.md .agents/skills/aidlc-cli/SKILL.md", false}, {"cat code.go", false}, {"/opt/aidlc intent plan " + st.ID + " --space default", false}, {"/opt/aidlc intent plan-approval " + st.ID + " --space default --expect " + strconv.FormatUint(st.Revision, 10) + " --file decision.json", false}} {
