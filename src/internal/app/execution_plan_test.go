...
func TestExecutionPlanCLIPlanAndPrompt(t *testing.T) {
	...
	r := cli.CommandRequest{Command: "intent", Action: "plan", Target: st.ID, Space: "default"}
...
func TestExecutionPlanCLILegacyAdvance(t *testing.T) {
	s, st := executionCLIFixture(t)
	_, err := s.Execute(cli.CommandRequest{Command: "intent", Action: "advance", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10)})
...
func TestExecutionPlanCLIStrictDraft(t *testing.T) {
	...
	if _, err = s.Execute(cli.CommandRequest{Command: "intent", Action: "plan", Target: st.ID, Space: "default", Expect: strconv.FormatUint(st.Revision, 10), File: file}); err == nil {
...
func TestOKFSkillReadApprovalPending(t *testing.T) {
	s, st := executionCLIFixture(t)
	store := flow.Store{Root: s.Root, Space: "default"}
	var err error
	st, err = store.Begin(st.ID, st.Revision)
	...
	for _, tc := range []struct {
		command string
		deny    bool
	}{{"touch code.go", true}, {"cat .agents/skills/okf-agent-memory/SKILL.md", false}, {"cat code.go", false}, {"/opt/aidlc intent plan " + st.ID + " --space default", false}, {"/opt/aidlc intent plan-approval " + st.ID + " --space default --expect " + strconv.FormatUint(st.Revision, 10) + " --file decision.json", false}} {
		input := HookInput{Event: "PreToolUse", Session: "session", Turn: "A", Tool: "Bash", ID: "tool"}
		input.Input.Command = tc.command
		out, err := s.Hook(input)
		if err != nil {
