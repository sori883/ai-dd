...
func TestProcedureDriftBlocksDocumentRepair(t *testing.T) {
	...
	out := hook(t, s, "PreToolUse", "Bash", "repair", "/opt/okf create codekb/current-analysis --space default --body-file draft --actor process:test --type CurrentAnalysis --title Current --description Current", false)
	if !deny(out) {
