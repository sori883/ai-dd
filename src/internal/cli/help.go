...
var publicActions = map[string]string{
	"install": "codex", "space": "create list switch", "intent": "create list switch show configure check begin review advance pause resume reopen wait cancel", "unit": "claim result integrate confirm reassign", "memory": "create update show search rules check", "session": "bind inspect",
}
...
func Help(args []string) (string, bool) {
	...
	usage := map[string]string{
		...
		"intent/show": "ID --space SPACE", "intent/check": "ID --space SPACE [--boundary start|end]", "intent/begin": "ID --space SPACE --expect REVISION", "intent/configure": "ID --space SPACE --expect REVISION --file CONFIG.json",
	...
	if key == "intent/check" || key == "intent/begin" {
		text += "checkは読取り専用。--boundary start|end（省略end）。beginは開始入力版を保存し同段階の再試行では差し替えない。一般作業/Unit claim前にbeginが必要。文書不足はWORKFLOWの必須型/節に従いmemory CLIで修復する。\n"
	}
	if key == "intent/configure" {
		text += "新規stateはschema_version=2。旧schemaは保持して明示エラー。material_sources/feature_knowledge/test_resultsはrepository相対pathの文字列配列、no_materials_reasonは文字列。material_sourcesは明示UTF-8ファイル/ディレクトリ。空ならdiscovery終了までにno_materials_reasonが必要。変更時はdiscoveryのbeginを無効化し、後段ではdiscoveryへのreopenが必要。entry/acceptedはCLI所有でconfigure不可。feature_knowledgeは統合終了で最低1件、共有解析/図とは別のknowledge/<機能名>.md。test_resultsはstrict JSONのstage(tdd|integration)/runs配列。現在存在する結果だけを指定し、次段階の結果は作成後に追記する。TDDはUnitごとのcommandとResultCommitの成功、integrationは全Unit統合後の現在HEADで計画command全ての成功を要求する。各runはcommand、実成果commit、必須整数exit_code、非空出力ファイルoutput_path。WORKFLOWの本文節と作成例に従う。\n"
