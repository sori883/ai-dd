...
func Help(args []string) (string, bool) {
	...
	if key == "install/codex" {
		text += "移転: aidlc install codex --relocate --project-dir NEW_ROOT --from-project-dir OLD_ROOT --from-binary OLD_BINARY\n新binaryは実行中のaidlc。旧pathは絶対参照文字列で存在不要。移転先AI開始前に端末から実行する。既知のaidlc/aidlc-cli/okf-agent-memoryの3skillと製品hooksの4ファイルを事前検査し参照だけを更新する。独自hookや旧WORKFLOW、Knowledgeを保持し、旧版のupgradeを兼ねない。部分失敗はPathsが更新済み、Pendingが未処理。全件再検査する同じ引数の再試行で復旧する。未知編集は自動上書きしない。新ROOT/.codex/hooks.jsonの絶対pathを確認し、Codexのhook trustを利用者が確認する。trust/認証設定は変更しない。\n"
	}
	if key == "unit/claim" {
...
initialization:
  開始: Rule、固定workflow、配置3skill（aidlc・aidlc-cli・okf-agent-memory）、hooks設定の有効なJSONを確認する。
