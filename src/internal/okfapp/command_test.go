...
func bodyRequest(file string) okfcli.CommandRequest {
	return okfcli.CommandRequest{Action: "create", Target: "codekb/note", Space: "default", BodyFile: file, Actor: "process:test", Metadata: okfmemory.MetadataInput{Type: bodyString("Note"), Title: bodyString("Memory"), Description: bodyString("Example"), ExtraJSON: bodyString(`{"custom":"keep"}`)}}
}
func TestOKFMemoryContract(t *testing.T) {
	s := setup(t)
...
func TestOKFSaveFailure(t *testing.T) {
...
func setup(t *testing.T) Service {
	...
	if _, err := (flow.Store{Root: root, Space: "default"}).Create("Work"); err != nil {
		t.Fatal(err)
	}
	return Service{Root: root}
}
func TestOKFConcurrentUpdate(t *testing.T) {
	s := setup(t)
	body := filepath.Join(s.Root, "body.md")
	...
	if err := os.WriteFile(body, []byte("Second"), 0600); err != nil {
		t.Fatal(err)
	}
	r.Action = "update"
	r.Expect = result["hash"]
	...
	for range 2 {
		go func() { _, err := s.Execute(r); results <- err }()
	}
	success := 0
	for range 2 {
		if <-results == nil {
	...
	if success != 1 {
		t.Fatalf("successful concurrent updates %d, want 1", success)
	}
}
