...
func TestExecutionPlanReviewHistoryMutation(t *testing.T) {
	for _, tc := range []struct{ mode, action string }{{"missing", "configure"}, {"hash", "configure"}, {"old head", "configure"}, {"invalid record", "configure"}, {"invalid UTF8", "configure"}, {"hash", "pause"}, {"hash", "source"}} {
		mode, action := tc.mode, tc.action
