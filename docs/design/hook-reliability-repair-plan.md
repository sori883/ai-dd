...
①の一時記録 `Session.Tool` は、会話内で実行中の操作IDを一つ持ちます。修正前のhookは保存用ロックを一度取得し、競合したら直ちにエラーを返していました。この経路はG0で再現し、coreではhookに限定した最大2秒の再試行を実装しました。`PostToolUse` のIDが保持中IDと一致した場合だけ解除・保存する境界は維持します。過去の実案件の原因がこの競合だったかは未確定です。
...
②の修正前の `child_hook.go` は、子の `Bash` と `apply_patch` 以外を拒否していました。coreでは現在工程の適格な登録から親宛を導出する専用分岐を追加しました。親側で同じ名前のツールを「子への追加依頼」として扱う既存経路は維持します。子のhookに届く `session_id` は親と共有されるため、個々の子の本人認証には使用しません。

③は固定Codex CLI **0.153.4 / macOS arm64** で、linked worktreeでは製品の5種類のhookが一覧に出ず、同じパス・資材を独立cloneへ移すと列挙・信頼確認できた実測があります。追加の固定source調査で、linked worktreeのhookは主checkout側を参照することが確認できました。今回は調査結果と配置上の制約を記録する範囲で完了とし、環境や製品の探索仕様は変更しません。
...
実機開始前にfixtureの絶対パス・配置内容・Intentの準備状態を提示し、通常のhook信頼確認を済ませます。子報告を調べる入口だけは、初回の `assignment init` に必要な人間確認と初期化完了も要求します。Bash・patch等の非agent試験はこの管理初期化へ依存させません。テスト一件ごとの承認は求めず、信頼対象が変われば新しい対象を示します。準備・確認待ちは実機25分の計測外です。
...
③は固定sourceの調査と制約記録で完了し、製品変更ファイルはありません。新しい診断CLIや常設の稼働台帳は追加しません。

## 順序付きTDDと検証分担

TDDは、変更前に失敗するテストを確認してから最小の修正を行う方式です。既に通る保護条件は回帰確認にし、REDを捏造しません。G0とcoreの対象testは追加・実行済みです。最新のRED／GREENと残る実機確認は作業単位の証拠RAMへ記録します。
...
完了は課題別に判定します。①は確認できた終了経路で不要な残存がなく、不明時の保護が残ること。②は途中通知が親に実際に届き、共有変更・再委譲を許可しないこと。③は固定sourceの探索挙動、既存実測との関係、配置制約と未実施の変更を調査RAMへ記録することで、今回の調査を完了とします。実機の新たな発火確認や環境修正を③の今回の完了条件に追加しません。Codex側の未解消制約を「修正済み」にしません。
