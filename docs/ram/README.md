...
| 意思決定 | [Codexのcontext読込をGoの安全境界へ移す](decisions/2026-09-05-codex-safe-context-read-contract.md) | In Progress（Issue #115。実装・再レビュー修正中、未merge／compact receipt repair後live未実施。publication generation、plan-wide cross-file commitment、bounded streaming、canonical token、read-only snapshotを含む） |
