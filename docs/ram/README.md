...
- [ステージ別担当の起動制限とworkerのworktree排他の計画](decisions/2026-09-10-stage-agent-worker-guard-planning.md): 計画のみ・実装/実機未承認。[具体計画](../design/stage-agent-worker-guard-plan.md)に固定Codexの前提gate、実行予約・停止/再開・競合・TDD・配布を整理。stage-plannerの許可段階、Unitなし予約、複数調整rootの3点を確認中。
