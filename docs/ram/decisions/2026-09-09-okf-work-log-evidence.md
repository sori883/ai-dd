...
pendingには保存前の`log_hash`と完成後の`log_after_hash`を保存する。固定した`at`で完成版を組み立て、
完成hash一致時の再試行は文書を書き直さずstateを確定する。旧pendingは完成hashがないため拒否する。
...
2. 保存途中・再試行
   ...
   - GREEN: 同command exit 0。各保存境界の失敗、同一要求の1回追記、別要求拒否、完成文書bytesと時刻の保持を確認。
