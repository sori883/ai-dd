...
relocateは現在版のaidlc/aidlc-cli/okf-agent-memoryの3skillと製品hooksの4ファイルを事前検査し、既知の参照だけを更新し、既存WORKFLOW・独自hook・Knowledgeを保持します。
...
aidlcスキルは進行・承認規約とこの読取りを案内し、aidlc-cliは操作目的からhelpへ案内し、okf-agent-memoryは知識の検索・保存を案内する。段階変更・再開後に現在手順を取り直す。
