...
`AdvanceContinuation`はStage、scope、bundle、directive hash、route hashと、state-aware時のstate hashを先に
...
工程のinline/mob担当AIへ渡すpersona・知識の表示pathを毎回組み立てます。readerはrootを探索・open/Closeせず、
`FrameworkDir`はplugin metadata warningの絶対表示専用です。開発用`src/core/`やRAMを実行時に読まず、本文を
返却・cache・binaryへ埋め込みません。

候補はpersona→framework共通→framework担当AI別→Space共通→Space担当AI別の5群です。各directoryはUTF-16
code-unit順の深さ優先でexact `.md` regular file/symlinkを列挙し、Minimal/pluginの絞込みより前に毎回
...
frontmatter内部のraw `\r\r\n`保持、inner horizontal whitespace runのline terminator / 終端backtracking、
nil FS、no-cacheとslice・runner pointerのcaller ownershipを固定します。blockが成立してitem抽出がemptyに
なった場合もflowへfallbackしないことを含みます。
...
各chunkのUTF-8境界、content digest、active marker、publication generation、record private HMAC tokenを検証し、改ざん・stale・内容変更・symlink・
